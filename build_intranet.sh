...
if [ -d "frontend/dist" ]; then
    echo "Frontend dist directory exists. Skipping build to avoid platform conflicts."
    ...
    if command -v npm &> /dev/null; then
        ...
        echo "Warning: npm not found in this environment. Skipping frontend build."
        if [ ! -d "frontend/dist" ]; then
            echo "Error: frontend/dist not found and npm is missing. Cannot proceed."
            exit 1
        fi
    fi
else
...
if [ -d "frontend/dist" ]; then
    ...
    rm -rf agent-ui
    mkdir -p agent-ui
    cp -r frontend/dist/* agent-ui/
    
    "$PYTHON_BIN" <<EOF_ZIP
import shutil
shutil.make_archive('agent-ui', 'zip', 'agent-ui')
EOF_ZIP
