...
### 🎯 目标
修复 500 错误、解决内网 NC 框架嵌入后的文件预览与端口访问问题、优化构建脚本以符合生产环境交付标准。

### ✅ 修复与优化 (Fixes & Optimizations)

#### 1. 核心 Bug 修复
*   **500 API Error**: 修复了向量数据库维度不匹配问题（1536 vs 1024）。通过重建数据库表解决了 `pgvector` 维度错误。
*   **文件预览 404**: 修复了前端构建产物未正确同步到后端静态目录导致的文件预览失败。
*   **构建脚本缺失文件**: 补充了 `validate_imports.py`，解决了 `build_intranet.sh` 执行报错的问题。

#### 2. 架构与网络适配
*   **双路访问支持**:
    *   **NC 框架嵌入**: 保持通过 Nginx 代理 `/m/demo/agent-ui/` 的访问方式。
    *   **直连访问**: 修复了后端 `StaticFiles` 挂载路径，现在可以直接通过 `http://IP:9020` 完整访问助手功能（含静态资源）。
*   **访客模式增强**: 修改 `App.jsx`，针对 Guest 用户隐藏了“运行简报”、“进化历程”、“数据看板”等敏感菜单。

#### 3. 构建系统重构 (Build System)
*   **脚本优化 (`build_intranet.sh`)**:
    *   **自动清理**: 构建完成后自动删除 `build_env`, `dist`, `build`, `*.spec` 等中间产物，保持目录整洁。
    *   **产物精简**: 确保只输出三个核心文件：`ops-agent-linux-x64.zip` (后端), `demo.zip` (NC模块), `agent-ui.zip` (前端资源)。
*   **NC 模块包规范化 (`demo.zip`)**:
    *   **结构修复**: 恢复了 `demo/` 目录的标准结构（包含 Vue 外壳组件）。
    *   **文件过滤**: 在打包时自动剔除 `Menu1`, `Menu2`, `TestPage` 等开发测试文件，仅保留生产环境组件。

### 📂 产物清单
*   `ops-agent-linux-x64.zip`: 后端全量包（含 Python 运行时 + 静态资源）。
*   `demo.zip`: NC 框架专用部署包（Vue Shell + React UI，无测试文件）。
*   `agent-ui.zip`: 纯 React 前端资源包。
